...
export function rewriteTrafficMarkdown(
  traffic: CodexProtocolTraffic,
  context: MarkdownRewriteContext
): CodexProtocolTraffic {
  if (traffic.kind === "request") {
    return rewriteRequestMarkdown(traffic, context);
  }
  if (traffic.kind === "event") {
    return rewriteEventMarkdown(traffic, context);
  }
  return traffic;
}

function rewriteRequestMarkdown(
  traffic: Extract<CodexProtocolTraffic, { kind: "request" }>,
  context: MarkdownRewriteContext
): CodexProtocolTraffic {
  if (traffic.method !== "turn/start") {
    return traffic;
  }
  const params = traffic.params as CodexRequestParams<"turn/start"> | undefined;
  if (!params) {
    return traffic;
  }
  ...
  return {
    ...
    params: {
      ...params,
      input: rewriteUserInputMarkdownArray(params.input, context)
    }
  };
}
  ...
  if (event.method === "item/agentMessage/delta" || event.method === "item/plan/delta") {
    return {
      ...
      event: {
        ...
        params: {
          ...event.params,
          delta: rewriteMarkdownAssetUrls(event.params.delta, context)
        }
      } as CodexProtocolEvent
    };
  }
...
function rewriteThreadItemMarkdown(
  ...
  switch (item.type) {
    case "userMessage":
      return {
        ...item,
        content: rewriteUserInputMarkdownArray(item.content, context)
      };
    case "agentMessage":
    case "plan":
      return {
...
function rewriteRolloutPayloadMarkdown(
  ...
  if (payload.type === "message" && (payload.role === "assistant" || payload.role === "user") && Array.isArray(payload.content)) {
    return {
      ...payload,
...
function rewriteUserInputMarkdownArray(
  value: readonly CodexParsedUserInput[],
  context: MarkdownRewriteContext
): CodexParsedUserInput[] {
  return value.map((entry) => rewriteUserInputMarkdown(entry, context));
}

function rewriteUserInputMarkdown(
