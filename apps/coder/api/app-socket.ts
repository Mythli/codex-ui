...
export function appSocketPlugin(path?: string): Plugin {
  ...
  return {
    ...
    configurePreviewServer(server) {
      installCodexAssetRoutes(server, dependencies.assets);
    }
  };
}
...
function installCodexAssetRoutes(server: ViteDevServer | PreviewServer, codexAssets: CodexAssetRegistry) {
  server.middlewares.use(`${codexAssets.routeBase}/register`, (request, response) => {
    ...
    request.on("end", () => {
      try {
        ...
        const registered = typeof payload.dataUrl === "string"
          ? codexAssets.registerDataUrl(payload.dataUrl, { originalName })?.asset
          : typeof payload.path === "string"
            ? codexAssets.registerFile(payload.path, { mimeType, originalPath: payload.path })
            : undefined;
