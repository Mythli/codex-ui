...
function completedFileChangeArtifact(turn: CodexTranscriptTurn, cwd?: string): CodexFileChangeEntry | undefined {
  const aggregate = aggregateFileChangeArtifact(turn, cwd);
...
function mergeFileChangeEntries(
  ...
  for (const file of left.files) {
    filesByPath.set(canonicalFileChangePathKey(filesByPath, file.path, cwd), file);
  }
  for (const file of right.files) {
    const key = canonicalFileChangePathKey(filesByPath, file.path, cwd);
...
function canonicalFileChangePathKey(
  filesByPath: ReadonlyMap<string, CodexTranscriptFile>,
  path: string,
  cwd: string | undefined
): string {
  const key = fileChangePathKey(path, cwd);
  for (const existingKey of filesByPath.keys()) {
    if (sameFileChangePath(existingKey, key)) {
      return existingKey;
    }
  }
  return key;
...
function aggregateFileChangeArtifact(turn: CodexTranscriptTurn, cwd?: string): CodexFileChangeEntry | undefined {
