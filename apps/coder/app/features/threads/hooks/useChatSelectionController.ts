...
import { shouldAutoSelectFirstChat } from "../state/hydrationGuards";
  ...
  const submitPromptFromSelection = useCallback(() => {
    void submitPrompt().then((result) => {
      if (result?.createdThreadId) {
        ...
        selectChat(result.createdThreadId, selection.projectId);
        onSelectChatRoute?.(result.createdThreadId);
      }
    });
  }, [onSelectChatRoute, selectChat, selection, submitPrompt]);
