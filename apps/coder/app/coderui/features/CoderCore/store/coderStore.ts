...
export const useCoderStore = create<CoderStore>((set, get) => ({
  ...
  deleteChat: async (chatId) => {
    ...
    await runtime.actions.archiveThread(chatId);
    set((state) => ({
      hydratingThreadIds: state.hydratingThreadIds.filter((id) => id !== chatId),
      unreadThreadIds: state.unreadThreadIds.filter((id) => id !== chatId)
    }));
  },
